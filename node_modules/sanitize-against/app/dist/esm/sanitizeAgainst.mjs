...
export class OBJECT extends Matcher {
...
export class AWAITED extends Matcher {
...
// boolean to throws
export function ensure(validate, msg) {
    return (input) => {
        if (!validate(input))
...
export const numberLikeStringPattern = new AND(numberLikeCommon, s => s + "");
export const numberLikePattern = new AND(numberLikeCommon, s => +s);
export function stringStartsWith(s) {
    return ensure((input) => input.startsWith(s), `Input must start with ${s}`);
}
export function numericRange(lowerBound, upperBound) {
...
export const nonEmptyStringPattern = new AND(String, ensure((input) => input !== "", "Input must be a non-empty string"));
export const any = (a) => a;
export const unknown = (a) => {
